    ...
    def __init__(self, device,model_name):
        ...
        if model_name == "attn-only-4l":
            ...
            for century in range(11, 18):
                ...
                self.YEARS_BY_CENTURY[century] = all_success[1:-1]
        if not self.YEARS:
            print(f"警告：模型 {model_name} 没有符合条件的年份，使用默认年份列表")
            # 添加一些默认年份
            for century in range(11, 18):
                for year in range(century * 100 + 2, (century * 100) + 99, 5):  # 每隔5年取一个
...
def greaterthan_accuracy(logits, tokens, model_name, return_one_element: bool=True):
    ...
    if return_one_element:
        # 返回负值，以便与代码中的 -test_metric(logits).item() 配合使用
        # 这样显示的准确率会是正值
        return -correct.mean()  
    else:
        return -correct  
...
def get_year_data(num_examples, model, model_name):
    ...
    # 确保 constants.YEARS 不为空
    if len(constants.YEARS) == 0:
        print(f"警告：模型 {model_name} 的 YEARS 列表为空，使用默认年份")
    ...
    # 使用 tokenizer 的 batch 处理功能，自动处理填充
    prompts_tokenized = model.tokenizer(
        prompts, 
        padding=True,  # 添加填充
        truncation=True,  # 如果需要，进行截断
        return_tensors="pt"
