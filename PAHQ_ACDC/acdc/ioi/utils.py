...
def get_all_ioi_things(num_examples, device, metric_name, kl_return_one_element=True, model_name="gpt2"):
    #tl_model = get_gpt2_small(device=device)
    tl_model = load_model(device=device,model_name=model_name)
    ...
    # 使用 io_tokenIDs 作为标签，它们现在应该是句子末尾的 token ID

    
    # 删除断言检查，因为我们已经确保 io_tokenIDs 是正确的
    ...
    labels = torch.as_tensor(ioi_dataset.io_tokenIDs[:num_examples*2], dtype=torch.long, device=device)
    wrong_labels = torch.as_tensor(ioi_dataset.s_tokenIDs[:num_examples*2], dtype=torch.long, device=device)
