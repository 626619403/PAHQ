...
parent_dir = os.path.abspath(os.path.join(os.path.dirname(current_file_path), os.pardir))
...
parser.add_argument('--wandb-entity-name', type=str, required=False, default=None, help='Value for WANDB_ENTITY_NAME')
...
parser.add_argument("--wandb-dir", type=str, default="/root/autodl-tmp/wandb")
parser.add_argument("--model_name", type=str, default="attn-only-4l")
...
parser.add_argument("--verbose", type=bool,default=True, help='Use to see the information')
if ipython is not None:
...
if TASK == "ioi":
    ...
    things = get_all_ioi_things(
        num_examples=num_examples, device=DEVICE, metric_name=args.metric, model_name=args.model_name,
    )
    test_metric = things.test_metrics["frac_correct"]
elif TASK == "or_gate":
    num_examples = 1
    seq_len = 1
    ...
    things = get_all_induction_things(
        num_examples=num_examples, seq_len=seq_len, device=DEVICE, metric=args.metric, model_name=args.model_name
    )
elif TASK == "docstring":
    num_examples = 50
    seq_len = 41
    things = get_all_docstring_things(
        ...
        device=DEVICE,
        metric_name=args.metric,
        correct_incorrect_wandb=True, model_name=args.model_name
    )
    test_metric = things.test_metrics["docstring_stefan"]
elif TASK == "greaterthan":
    num_examples = 100
    things = get_all_greaterthan_things(
        num_examples=num_examples, metric_name=args.metric, device=DEVICE, model_name=args.model_name
    )
    test_metric = things.test_metrics["accuracy"]
else:
...
tl_model = things.tl_model.to("cuda") # transformerlens model

if RESET_NETWORK and TASK!="or_gate":
    reset_network(TASK, DEVICE, tl_model)
    if TASK=="or_gate":
        print("RESET_NETWORK for or_gate task is not available")
