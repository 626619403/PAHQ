...
#python roc_plot_generator.py --task=docstring --metric=kl_div --alg=acdc --skip-sp --skip-sixteen-heads
...
if IPython.get_ipython() is not None:
    ...
    if "arthur" not in __file__:
        __file__ = "./notebooks/roc_plot_generator.py"
else:
...
# # for SP # filters are more annoying since some things are nested in groups
SP_PROJECT_NAME = "induction-sp-replicate"
SP_PRE_RUN_FILTER = {
    "state": "finished",
    "config.task": TASK,
...
SIXTEEN_HEADS_PROJECT_NAME = "acdc"
...
if TASK == "docstring":
    ...
    things = get_all_docstring_things(num_examples=num_examples, seq_len=seq_len, device=DEVICE,
                                                metric_name=METRIC, correct_incorrect_wandb=True)
    ...
    if not ZERO_ABLATION:
        ACDC_PROJECT_NAME = "626619403-none/acdc"
    ...
    SP_PROJECT_NAME = "mouse-nest/induction_arthur"  # moved here manually
...
# if True:
    ...
    for run in filtered_runs:
        ...
        print("ACDC_PRE_RUN_FILTER:", ACDC_PRE_RUN_FILTER)
        print("ACDC_RUN_FILTER:", ACDC_RUN_FILTER)
...
if not SKIP_ACDC: # this is slow, so run once
    ...
    print("acdc_corrs length:", len(acdc_corrs))
    print("acdc_corrs content:", acdc_corrs)
