...
class TLACDCExperiment:
    ...
    def __init__(
        ...
        ds: torch.Tensor,
        test_metric : Callable[[torch.Tensor], torch.Tensor],
    ...
    def add_sender_hook(self, node, override=False):
        ...
        self.model.add_hook(
            name=node.name, 
            hook=partial(self.sender_hook, verbose=self.hook_verbose, cache="online", device="cpu" if self.online_cache_cpu else "cuda"),
        )
    ...
    def step(self, early_stop=False, testing=False):
        ...
        if self.test_metric is not None:
            #old_hooks = {}
            #for name, hook_point in self.model.hook_dict.items():
                #old_hooks[name] = hook_point.fwd_hooks.copy()
                #hook_point.fwd_hooks = []
            
            with torch.no_grad():
                logits = self.model(self.ds)
            accuracy = -self.test_metric(logits).item()
            
            #for name, hooks in old_hooks.items():
                #self.model.hook_dict[name].fwd_hooks = hooks
            
            if self.verbose:
                print(f"Step {self.step_idx} accuracy: {accuracy:.4f}")
            
            if self.using_wandb:
                wandb.log({"accuracy": accuracy, "step": self.step_idx})
    ...
    def current_node_connected(self):
        ...
        self.update_cur_metric(recalc_metric=True,recalc_edges=True)
        #assert abs(self.cur_metric - old_metric) < 3e-2, ("Removing all incoming edges should not change the metric ... you may want to see *which* remooval in the above loop mattered, too", self.cur_metric, old_metric, self.current_node) # TODO this seems to fail quite regularly
        return False
    ...
    def increment_current_node(self) -> None:
        while True:
            ...
            if self.current_node is None or self.current_node_connected() or self.current_node.name in ["blocks.0.hook_resid_pre", "hook_pos_embed", "hook_embed"]:
                if self.current_node != None and self.current_node.index.hashable_tuple:
                    
                    node_string = f"{self.current_node.name}[{', '.join(map(str, self.current_node.index.hashable_tuple))}]"
                    match = re.match(r"blocks\.(\d+)\.attn.*\[(?:.*?, ){2}(\d+)\]", node_string)
                    if match:
                        
                        layer = int(match.group(1))  
                        head = int(match.group(2))  

                        self.model.update_head_layer(layer, head)
                        if self.verbose:
                            print(f"Updated to layer {layer}, head {head}")
                break
            if self.verbose:        
                print("But it's bad")
