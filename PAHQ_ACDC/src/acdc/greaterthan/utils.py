...
class GreaterThanConstants:
    ...
    @classmethod
    def get(cls: type["GreaterThanConstants"], device,model_name='gpt2') -> "GreaterThanConstants":
        if cls._instance is None:
            cls._instance = cls(device, model_name)
    ...
    def __init__(self, device,model_name):
        ...
        if model_name == "attn-only-4l":
            # Use hardcoded year list (skip tokenizer check for this model)
            ...
            for century in range(11, 18):
                ...
                self.YEARS_BY_CENTURY[century] = all_success[1:-1]
        if not self.YEARS:
            print(f"Warning: model {model_name} has no qualifying years, using default year list")
            # Add some default years
            for century in range(11, 18):
                for year in range(century * 100 + 2, (century * 100) + 99, 5):  # sample every 5 years
                    self.YEARS.append(str(year))
                self.YEARS_BY_CENTURY[century] = [str(year) for year in range(century * 100 + 2, (century * 100) + 99, 5)]
...
def greaterthan_metric_reference(logits, tokens, model_name):
    constants = GreaterThanConstants.get(logits.device, model_name)
...
def greaterthan_metric(logits, tokens, model_name,return_one_element: bool=True):
    constants = GreaterThanConstants.get(logits.device,model_name)
...
def greaterthan_accuracy(logits, tokens, model_name, return_one_element: bool=True):
    constants = GreaterThanConstants.get(logits.device, model_name)
    probs = F.softmax(logits[:, -1], dim=-1)
    yearend = constants.INV_TOKENS_TENSOR[tokens[:, 7]].to(logits.device)
    predicted_numbers = torch.argmax(probs[:, constants.TOKENS_TENSOR], dim=1)

    correct = (predicted_numbers > yearend).float()
    
    if return_one_element:
        # Return negative so that -test_metric(logits).item() gives positive accuracy
        return -correct.mean()  
    else:
        return -correct  

'''def get_year_data(num_examples, model, model_name):
    constants = GreaterThanConstants.get(model.cfg.device ,model_name)
...
def get_year_data(num_examples, model, model_name):
    ...
    # Ensure constants.YEARS is not empty
    if len(constants.YEARS) == 0:
        print(f"Warning: YEARS list for model {model_name} is empty, using default years")
        years = [str(year) for year in range(1200, 1800, 10)]
    else:
    ...
    # Use tokenizer batch processing with automatic padding
    prompts_tokenized = model.tokenizer(
        prompts,
        padding=True,
        truncation=True,
        return_tensors="pt"
...
def get_all_greaterthan_things(num_examples, metric_name, model_name = "gpt2", device="cuda"):
    model = load_model(device=device,model_name=model_name)
    data, prompts = get_year_data(num_examples*2, model,model_name=model_name)
    ...
    if metric_name == "metric":
        validation_metric = partial(greaterthan_metric, tokens=validation_data.cpu(),model_name=model_name)
    ...
    test_metrics = {
        "greaterthan": partial(greaterthan_metric, tokens=test_data.cpu(),model_name=model_name),
        "accuracy": partial(greaterthan_accuracy, tokens=test_data.cpu(),model_name=model_name),
        "kl_div": partial(
            kl_divergence,
