...
def get_all_induction_things(num_examples, seq_len, device, model_name="gpt2", data_seed=42, metric="kl_div", return_one_element=True) -> AllDataThings:
    ...
    # Determine vocab size and max sequence length for safety checks
    if hasattr(tl_model, 'cfg'):
        ...
        vocab_size = getattr(tl_model.config, 'vocab_size', 50257)
        max_seq_len = getattr(tl_model.config, 'max_position_embeddings', 1024)
    ...
    with torch.no_grad():
        # Use batched inference for large datasets to avoid OOM
        ...
        if total_elements > 100000:
            ...
            for i in range(num_batches):
                ...
                all_val_logprobs.append(F.log_softmax(outputs, dim=-1).detach())
            ...
            for i in range(num_batches):
                ...
                outputs = tl_model(batch)
                all_test_logprobs.append(F.log_softmax(outputs, dim=-1).detach())
