...
# 在 IOIDataset 类中添加以下方法



# 然后在 __init__ 中使用这个方法
...
class IOIDataset:
    def __init__(
        ...
        io_positions, s_positions = self.find_token_positions()
        self.io_tokenIDs = io_positions
        self.s_tokenIDs = s_positions
    ...
    def find_token_positions(self):
        """找到 IO 和 S 在实际分词后的位置"""
        io_positions = []
        s_positions = []
        
        for i, prompt in enumerate(self.ioi_prompts):
            # 获取完整文本的 token IDs
            full_text = (self.tokenizer.bos_token if self.prepend_bos else "") + prompt["text"]
            tokens = self.tokenizer.encode(full_text)
            
            # 对于 IOI 任务，IO 应该是最后一个 token
            io_positions.append(tokens[-1])
            
            # 获取 S 的 token ID
            s_text = " " + prompt["S"]
            s_tokens = self.tokenizer.encode(s_text, add_special_tokens=False)
            
            # 在完整文本中查找 S
            s_found = False
            for j in range(len(tokens) - len(s_tokens) + 1):
                if tokens[j:j+len(s_tokens)] == s_tokens:
                    s_positions.append(tokens[j])
                    s_found = True
                    break
            
            if not s_found:
                s_positions.append(s_tokens[0])
        
        return io_positions, s_positions
