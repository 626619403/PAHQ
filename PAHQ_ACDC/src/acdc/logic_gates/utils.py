...
def test_and_logical_model():
    ...
    seq_len=3
    and_model = get_logic_gate_model(mode="AND", seq_len=seq_len, device = default_device)
...
def get_all_logic_gate_things(mode: str = "AND", device=default_device, seq_len: Optional[int] = 5, num_examples: Optional[int] = 10, return_one_element: bool = False) -> AllDataThings:

    assert mode == "OR" 

    model = get_logic_gate_model(mode=mode, seq_len=seq_len, device=device)
    # Convert the set of binary string back llto tensor
    data = torch.tensor([[0.0]]).long().to("cuda") # Input is actually meaningless, all that matters is Attention Heads 0 and 1
