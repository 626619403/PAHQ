...
class HookedTransformer(HookedRootModule):
    ...
    def update_W_low(self):
        ...
        transformer_block_list = self.blocks
        for i, block in enumerate(transformer_block_list):
            block.attn.update_W_low()
            
    def update_head_layer(self, layer, head):
        '''
        Update the selected head in the selected layer'''
        self.blocks[self.selected_layer].update_selected_head(selected_head=-1)
        self.selected_layer= layer     
        self.blocks[layer].update_selected_head(selected_head=head)
